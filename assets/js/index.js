...
function list(){
var t1_o = document.querySelectorAll('.table_1_option option');
...
function add_medicoment() {
  ...
  table_2.innerHTML += '<option value="'+this.value+'">'+this.text+'</option>';
  summa_medicoment=summa_medicoment+parseInt(this.text);
...
function remove_medicoment() {
  this.classList.toggle('list_none');  
  table_1.innerHTML += '<option>'+this.text+'</option>';
  summa_medicoment=summa_medicoment-parseInt(this.text);
  r_s_m.innerHTML = summa_medicoment;
  var t1_o = document.querySelectorAll('.table_1_option option');
  for (var i = 0; i < t1_o.length; i++) {
